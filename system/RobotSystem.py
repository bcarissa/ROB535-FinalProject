...
class RobotSystem:
    ...
    def run_filter(self):
        ...
        for t in range(4):
            ...
            #     X, P= self.filter_.prediction(motion_command, X, P, t)
            #     X, P = self.filter_.correction(observation, self.landmarks, X, P)
                    
            # elif self.filter_name == 'UKF':
            #     Y, w, X, P= self.filter_.prediction(motion_command, X, P, t)
            #     X, P = self.filter_.correction(observation, self.landmarks, Y, w, X, P)
            
            # elif self.filter_name == 'PF':
            #     particles= self.filter_.prediction(motion_command, particles, t)
            #     X, P, particles, particle_weight = self.filter_.correction(observation, self.landmarks, particles, particle_weight, t)
                    
            
            # elif self.filter_name == "InEKF":
            #     mu_pred, sigma_pred = self.filter_.prediction(motion_command, P, mu, t)
            #     X, P, mu = self.filter_.correction(Y, Y2, observation, self.landmarks, mu_pred, sigma_pred)
